...
<!--            </div>-->
            ...
            <div class="col-md-6">
              <div>
                <app-typography variant="body1" class="control-label mb-2">
  ...
  <common-footer @back="goBackHandler" @continue="handleSubmit"/>
</template>
...
export default {
      ...
      addressValidationRules: [
        {
          ...
          message: 'Please enter an address.', // Updated message
        },
        {
          validator: (value) => {
      ...
      dobValidationRules: [
        {
          ...
          message: 'Please enter a valid date of birth.', // Updated message
        },
      ],
      userNameValidationRules: [
        {
          regex: /^[a-zA-Z0-9_]+$/,
          message: 'Please enter a valid username.', // Updated message
        },
      ],
    };
  },
  ...
  methods: {
    initializeFormData() {
      if (this.userProfile) {
        ...
        // Fetch cities based on the saved state
        if (this.formData.state) {
          const stateSlug = this.states.find(state => state.slug == this.formData.state)?.slug;
          if (stateSlug) {
            this.handleStateChange(stateSlug); // Fetch cities for the saved state
          }
        }
    ...
    validateInput(inputField) {
      switch (inputField) {
        ...
        case "date_of_birth":
          ...
          if (!error && this.formData.date_of_birth) {
            // Convert DD-MM-YYYY to YYYY-MM-DD for Date parsing
            const [day, month, year] = this.formData.date_of_birth.split('-');
    ...
    handleCountryChange(countrySlug) {
      this.$inertia.get('/profile', { country: countrySlug }, {
        preserveState: true,
  ...
  handleStateChange(stateSlug) {
    ...
    this.$inertia.get('/profile', { state: stateSlug }, {
      preserveState: true,
      preserveScroll: true, // Prevent scrolling up
      only: ['cities'],
      onSuccess: (response) => {
  ...
  handleSubmitProfileImage() {
    ...
    this.profile.post('/profile', formData, {
